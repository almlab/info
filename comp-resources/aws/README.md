...
Lots of people in the Alm lab do computational work.
People use different resources, but most of the computational resources
funded by the lab are through Amazon Web Services (AWS).

AWS is administrated by Claire Duvallet (duvallet[at]mit[dot]edu) and Xiaofang Jiang (xiaofang[at]mit[dot]edu).
The Alm lab pays for everything related to our AWS usage, and Claire and Xiaofang
are basically volunteering their time to administer this common resource.

We communicate about AWS and computing-related issues via [slack](https://almstars.slack.com/) on the `#aws` channel.
...
AWS is pretty bare bones: it's really just a bunch of data centers containing CPUs+RAM (EC2)
...
If you are not actively using data, please move it to S3. From the almlab node, you
...
If you need to install new software that will be generally useful to the lab, you can put it in `/home/ubuntu/bin` (if it is a binary) or `/home/ubuntu/tools` (if it is an executable).
...
you safeguard your work so that you can continue your work even if someone (including you) accidentally does something bad.
...
resource.
The `#aws` slack channel is a really good place to have these conversations!
...
[Dropbox Business](https://ist.mit.edu/dropbox), which has a lot of storage for free.
...
If you're using something like ```screen``` or ```tmux```, make sure to name your sessions and close them out when you're finished.
...
These instances can be viewed as regular Linux-operated computers which can be accessed with an IP and an SSH key.  You can obtain both from the systems administrators (currently Claire and Xiaofang).  Note that both of these pieces of information are private and, for security reasons, should not be shared with anyone without explicit permission.  The SSH key should never be sent by email, and should reside somewhere safe on your computer.  Below we review the access procedures for accessing EC2 resources from Linux and Windows machines.
...
#### Troubleshooting access from Linux/MacOS

If you have previously logged onto an instance with that IP and the IP has now been associated with a different instance, your SSH manager will complain.  If you get an error, check your ~/.ssh/known_hosts and remove the line with that IP, then try again.

Your SSH manager may also complain if your key has the wrong permissions.
...
then you can change the permissions on the key file using `chmod`:
...
To upload/download files from S3 using a terminal, you will need to install the [AWS command line interface tools](https://aws.amazon.com/cli/) (CLI).  Once you have this installed, type
